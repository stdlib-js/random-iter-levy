...
function iterator( mu, c, options ) {
	...
	if ( !isNumber( mu ) || isnan( mu ) ) {
		throw new TypeError( format( '0pz6v,NI', mu ) );
	}
	if ( !isPositive( c ) ) {
		throw new TypeError( format( '0pz72,Go', c ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0pz2V,FD', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0pz2t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
